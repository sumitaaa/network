...
void blockErr(char ep[4], short int errMessage){
...
void uncodeErr(int r, char code[], char errMessage[128]){
	short int tmp, en ;
	memcpy(&tmp, code+2, 2);
	en = ntohs(tmp); 
	switch(en){
...
void blockRW(int r, char code[139],short int *opcode, char filename[128], char mode[9]){
...
int checkop(char blockIN[]){
	short int tmp;
	memcpy(&tmp, blockIN, 2);
	return ntohs(tmp);
}

void blockDP(char dp[516], short int no, char data[512]){
...
void checkACK(char code[4], short int *opcode, short int *blockno){
	short int tmp;
...
int main(){
	...
	if(checkop(rd)==1 || checkop(rd)==2){
		blockRW(r, rd, &opcode, filename, mode);
	}else if(checkop(rd)==5){
		//error
		char errMessage[128];
		uncodeErr(r, rd, errMessage);
	...
	if(opcode == 1){
		...
		while((byteread = read(fd, buf, sizeof(buf)))!=0){
			...
			if(checkop(ack)==5){
				//error
				char errMessage[128];
                		uncodeErr(r, ack, errMessage);
 		                printf("!!! got err message : %s!!!\n", errMessage);
				return 1;
			}else if(checkop(ack)==4){
				checkACK(ack, &opcode2, &blockno2);
