...
void checkErr(int r, char code[], char errMessage[128]){
        short int tmp, en ;
        memcpy(&tmp, code+2, 2);
        en = ntohs(tmp);
        switch(en){
...
void blockDP(char dp[516], int r, short int *opcode, short int *blockno, char data[512]){
	short int tmp;
	memcpy(&tmp, dp, 2);
	*opcode = ntohs(tmp);
	memcpy(&tmp, dp+2, 2);
...
void blockACK(char ack[4], short blockno){	
	short int tmp;
	tmp = htons(4);
	memcpy(ack, &tmp, 2);
	tmp = htons(blockno);
...
int blockRW(char bd[139], short int opcode, char filename[128], char mode[9]){
...
int main(){
	...
	char bd[139];
        int len = blockRW(bd, 1, "Read.txt", "octet");
	...
	if(checkop(dp)==5){
		//error
		char errMessage[128];
                checkErr(r, dp, errMessage);
                printf("!!! got err message : %s!!!\n", errMessage);
		return 1;
	}else if(checkop(dp)==3){
		blockDP(dp, r, &opcode, &blockno, data);
		...
		while(1){
			...
			if(checkop(dp)==5){
				...
				blockDP(dp, r, &opcode2, &blockno3, data);
				printf("got dp block = %d\n", blockno3);
				printf("========data==============\n%s\n===============\n", data);
				blockno2++;
			}
